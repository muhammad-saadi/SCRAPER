const puppeteer = require('puppeteer-extra');
const fs = require('fs');
const StealthPlugin = require('puppeteer-extra-plugin-stealth');

puppeteer.use(StealthPlugin())

const restaurant = { name: "The Rocks", pos_system: "None", url: 'https://postmates.com/store/the-rocks-tasting-room-and-liquor-store/nn6DK7ZXVC6mS4n2GkEvfg', scrape_type: "pdf_rip" };
(async () => {
  const browser = await puppeteer.launch({ headless: false });
  const page = await browser.newPage();
  await page.goto(restaurant.url, { waitUntil: 'networkidle2' });

  // await new Promise(resolve => setTimeout(resolve, 3000));
 

  const itemInfoSelectors = await page.$$eval('.md', contents => {
    return contents.flatMap(content => {
      const mealType = content.querySelector('.al').innerText;
      const items = content.querySelectorAll('.im.mr.ms.mt.ak.j5.f1.bm');
      return Array.from(items).map(item => ({
        anchorHref: item.querySelector('.al.ec.d8.dd.mu.mv.mw') ? item.querySelector('.al.ec.d8.dd.mu.mv.mw').getAttribute('href') : '',
        name: item.querySelector('.dl') ? item.querySelector('.dl').innerText : '',
        itemPrice: item.querySelector('.g2.fv.g3.be.bf.g4.bh.bi.b1') ? item.querySelector('.g2.fv.g3.be.bf.g4.bh.bi.b1').innerText : '',
        meal_type: mealType,
        description: item.querySelector('.be.bf.g4.bh.bi.b1') ? item.querySelector('.be.bf.g4.bh.bi.b1').innerText : '',
      }));
    });
  });

  // await page.waitForSelector('button[data-testid="close-button"]');
  

  let extractedData = [];

  console.log(`items:  ${itemInfoSelectors}`);
  for (const { anchorHref, name, itemPrice, meal_type, description } of itemInfoSelectors) {
    try {
      console.log(`Clicking on ${restaurant.url + anchorHref}`);
      console.log(`meal type: ${meal_type}`);
      await page.evaluate((href) => {
        const link = document.querySelector(`a[href="${href}"]`);
        if (link) link.click();
      }, anchorHref);

      // await page.evaluate(() => {
      //   const closeButtonSelector = document.querySelector('.al.ag.r1.dz.r2.e0.cj.cx.dd.fd.af.cn.m2.da.db.dc.eo.r3.r4.r5.cw.aq.cj.cx.cy.cz.d0.d1.d2.d3.d4.d5.d6.d7.d8.d9.da.db.dc.dd.de.hm.dg.dh.di.dj.dk.be.dl.bg.dm.dn.do.dp.dq.dr.ds.dt.du.b1.hw.hx.hy.dz.e0');
      //   if (loseButtonSelector) closeButtonSelector.click();
      // });
        // Wait for the modal to appear
      // await page.waitForSelector('.ar.kw.l0.jr.as.at.o2', { visible: true, timeout: 10000 });

      // Wait for 3 seconds to keep the modal open
      
      await new Promise(resolve => setTimeout(resolve, 3000));

      await page.evaluate(() => {
        const modalContent = document.querySelectorAll('div.al.ak.qy');
        if (modalContent) {console.log(`modal contentttt ${modalContent}`)}
        else {
          console.log('modal content not found');
        }
      });
      const modalText = await page.$$eval('.al.ak.qy', groups => {
        return groups.map(group => ({
            modifier: group.querySelector('.bg') ? group.querySelector('.bg').innerText : '',
            price: group.querySelector('.bo') ? group.querySelector('.bo').innerText : ""
        }));
      });
      console.log(`Modal text: ${modalText}`);

      extractedData.push({
        name: name,
        description: description,
        meal_type: meal_type,
        price: itemPrice,
        modalContent: modalText
      });

      // Close the modal (assuming there's a close button within the modal)
      await page.evaluate(() => {
        const closeButton = document.querySelector('cw.aq.cj.cx.cy.cz.d0.d1.d2.d3.d4.d5.d6.d7.d8.d9.da.db.dc.dd.de.hm.dg.dh.di.dj.dk.be.dl.bg.dm.dn.do.dp.dq.dr.ds.dt.du.b1.b0.dx.dy.dz.e0');
        if (closeButton) closeButton.click();
      });
      await page.waitForSelector('.ar', { hidden: true });
    } catch (error) {
      console.log(`Error while processing item with href ${anchorHref}: ${error.message}`);
    }
  }

  const filePath = `${restaurant.name.replace(/\s+/g, '_').toLowerCase()}_test7.json`;
  fs.writeFileSync(filePath, JSON.stringify(extractedData, null, 2), 'utf-8');
  console.log(`Extracted and saved item info for ${restaurant.name} at ${filePath}`);

  await page.close();

  await browser.close();
  console.log(`All done, check the JSON files. ✨`);
})();


// if (await page.$$eval('div.al.ak.qy')){
//   const modalText = await page.$$eval('div.al.ak.qy', groups => {
//     return groups.map(group => ({
//         modifier: group.querySelector('.be.bf.bg.bh.bi.g5.mz') ? group.querySelector('.be.bf.bg.bh.bi.g5.mz').innerText : '',
//         price: group.querySelector('.be.bf.g4.bh.bi.g5.bo') ? group.querySelector('.be.bf.g4.bh.bi.g5.bo').innerText : ''
//     }));
//   });
//   console.log(`Modal text: ${modalText}`);

//   extractedData.push({
//     name: name,
//     description: description,
//     meal_type: meal_type,
//     price: itemPrice,
//     modalContent: modalText
//   });
// }
// else {
//   extractedData.push({
//     name: name,
//     description: description,
//     meal_type: meal_type,
//     price: itemPrice,
//     modalContent: ""
//   });
// }

// await page.evaluate(() => {
//   const closeButton = document.querySelector('cw.aq.cj.cx.cy.cz.d0.d1.d2.d3.d4.d5.d6.d7.d8.d9.da.db.dc.dd.de.hm.dg.dh.di.dj.dk.be.dl.bg.dm.dn.do.dp.dq.dr.ds.dt.du.b1.b0.dx.dy.dz.e0');
//   if (closeButton) closeButton.click();
// });